...
export default function ClauseDiffViewer({ docId, initialRecommendations = [] }) {
  const [recs, setRecs] = useState(initialRecommendations);

  useEffect(() => {
    if (initialRecommendations && initialRecommendations.length) {
    ...
    fetchData();
  }, [docId, initialRecommendations]);

  return (
    <div className="diff-container">
      <h2>Clause Recommendations</h2>
      {recs.length === 0 ? (
        <p className="muted">No specific recommendations were generated for this upload.</p>
      ) : (
        recs.map((r, idx) => (
